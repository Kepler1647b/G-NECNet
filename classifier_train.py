...
from NETU_classifier.Dataloader_train import ZSData
...
def run(model, dataloaders, phase, device, criterion, optimizer):
    ...
    Writer.add_scalar('%s/NETU_Precision' % phase.capitalize(), p, global_epoch)
    Writer.add_scalar('%s/NETU_Recall' % phase.capitalize(), r, global_epoch)
    Writer.add_scalar('%s/NETU_F1' % phase.capitalize(), F1, global_epoch)
...
def start_train(train_loader, valid_loader, model, device, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print('NETU: t_precision: {:.4f}, t_recall: {:.4f}, t_F1: {:.4f}'.format(t_p, t_r, t_F1))
        print('NETU: v_precision: {:.4f}, v_recall: {:.4f}, v_F1: {:.4f}'.format(v_p, v_r, v_F1))
        if v_F1 > best_F1:
            best_F1 = v_F1
...
def prepare_data(dataset_path, batch_size=64):
    ...
    train_dataset = ZSData(os.path.join(dataset_path, 'train'), transforms=utils.transform_train, bi=True, padding=0)
    valid_dataset = ZSData(os.path.join(dataset_path, 'valid'), transforms=utils.transform_valid, bi=True, padding=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--TrainFolder", dest='train_folder', default='./data/stomach/Vahadane_stomach_split_10x_cancer', type=str, help='the datasets path')
    ...
    parser.add_argument("--NumEpoch", dest='num_epochs', default=150, type=int, help='the training epoch')
    ...
    parser.add_argument("--BatchSize", dest='batch_size', default=32, type=int, help='batch size')
    ...
    parser.add_argument("--Save_path", dest='save_path', default='../../NETUnet_checkpoints', help='the path to save the training result')
    ...
    global checkpoints
    checkpoints = os.path.join(args.save_path,
    ...
    train_loader, valid_loader, weight_per_class = prepare_data(os.path.join(args.train_folder, str(args.foldn)), args.batch_size)
